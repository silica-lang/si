...
impl Resolver {
    ...
    fn lower_stmt(&mut self, stmt: &Stmt, scope: &mut Scope, vars: &[SirVar], out: &mut Vec<SirStmt>) {
        match stmt {
            ...
            Stmt::Let(l) => {
                // §4.3: an explicit annotation types the binding (and is checked
                // against the initialiser's width/sign); otherwise infer.
    ...
    fn lower_expr_stmt(&mut self, expr: &Expr, scope: &mut Scope, vars: &[SirVar], out: &mut Vec<SirStmt>) {
        // §4.3 width/sign check over the whole statement expression (single pass).
